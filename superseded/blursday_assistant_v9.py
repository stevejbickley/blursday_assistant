...
def parse_assistant_response(response,thread_id,run_id,person_id):
    parsed_data = {
        'raw_data': [],
        'categories': [],
        'ranking': [],
    ...
    if not all(tags_found.values()):
        print('/n Not all tags found: ' + tags_found + '/n in response: ' + response + '/n for person_id: ' + person_id + ' with thread ID: ' + thread_id + ' and run ID: ' + run_id)
    else:
        parsed_data['dump'].append(response)
    return parsed_data
...
for i in range(0,len(created_thread_ids)):
    ...
    if os.path.exists(file_path):
        ...
        if thread_id in existing_data['thread_id'].unique():
    ...
    parsed_responses = [parse_assistant_response(message.content[0].text.value,thread_id,run.id) for message in messages]

    (response, thread_id, run_id, person_id)
    ...
    try:
        ...
        continue
    # Add the thread_id back into it
    df_responses['thread_id'] = thread_id
    df_responses['run_id'] = run.id
