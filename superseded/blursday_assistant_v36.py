...
#filtered_extract3 = extract_from_multiple_pages(not_in_analysis_df, base64_images, output_filename3, write_path, batch_size=32, index_number=-3)
...
file_path = './output_data/parsed_responses_extracted_clean_wide_for_analysis.xlsx' #'blursday_PastFluency-FutureFluency_2023-11-30_translated.csv'
analysis_df2 = pd.read_excel(file_path, dtype=str) # just read the full dataset
file_path = './output_data/parsed_responses_extracted2_clean_wide_for_analysis.xlsx' #'blursday_PastFluency-FutureFluency_2023-11-30_translated.csv'
...
analysis_df = analysis_df.dropna(subset=["Screen_Number", "original_response_lower"]) # 100443 rows down to 100258 rows
analysis_df = analysis_df.drop_duplicates(subset=['Experiment_ID', 'PID', 'UTC_yyyymmdd', 'Response_translated_lower', 'Screen_Number']) # 75353 rows
...
for main_category, subcategories in categories.items():
    ...
    ax.set_title(f"{main_category}: Mean Per Person by Sub-Category for Future Fluency and Past Fluency Tasks")
    ax.set_ylabel("Count")
    ax.set_xticks(x)
    ax.set_xticklabels(["1 year ago", "1 month ago", "1 week ago", "1 week ahead", "1 month ahead", "1 year ahead"])
    # ax.legend(title="Category", loc="upper left", bbox_to_anchor=(0, 1))  # Move legend to top left
    ...
    for condition in ["Past Fluency", "Future Fluency"]:
        ...
        if regression_type == "negative_binomial":
            ...
            results.append({
                ...
                "Pseudo R-squared": mcfadden_r_squared,  # Negative Binomial only...
                "nobs": model.nobs  # Add number of observations
            })
...
# Pivot the table
pivot_df = long_df.pivot(index="Predictor", columns=["Dependent Variable", "Condition"], values="Coefficient (p-value)")
...
metrics_pivot = metrics_df.pivot(index="Metric", columns=["Dependent Variable", "Condition"], values="Value")
...
def calculate_reliability_metrics_all(data, base_categories, suffixes):
    """
    Calculate reliability metrics for all categories and their variations (e.g., utilitarian, discretionary).
    Args:
        data (pd.DataFrame): The dataset containing ratings.
        base_categories (list): The base category names (e.g., ['category_utilitarian', ...]).
        suffixes (list): Suffixes for different raters (e.g., ['', '_x', '_y', '_z']).
    Returns:
        dict: Reliability metrics for each base category.
    """
    results = {}
    for base_category in base_categories:
        # Construct the full list of rating columns for the category
        rating_columns = [f"{base_category}{suffix}" for suffix in suffixes]
        # Drop rows with missing values in these columns
        complete_data = data[rating_columns].dropna()
        if complete_data.empty:
            print(f"No complete ratings available for {base_category}. Skipping.")
            continue
        # Create a count matrix for Fleiss' Kappa
        ratings = complete_data.apply(pd.Series.value_counts, axis=1).fillna(0)
        ...
        agreement = ((complete_data.apply(lambda x: len(set(x)) == 1, axis=1).mean()) * 100)
        # Store results
        results[base_category] = {
            "Fleiss' Kappa": fleiss,
            "Cohen's Kappa": cohens,
...
def generate_all_reliability_reports(file_paths, columns_of_interest, merge_keys, base_categories, suffixes_all=['', '_x', '_y', '_z'], suffixes_subset=['', '_0', '_1'], subset_indices=[0,2,3]): # By default, subset_indices is the humans only
    """
    Generate a detailed reliability report.
    """
    # Load and merge datasets
    ...
    merged_subset = merged_subset.drop_duplicates(subset=['Experiment_ID', 'PID', 'UTC_Date', 'Response_translated'])  # 1,179,345 rows down to 92,849 rows
    # Calculate reliability metrics for all raters
    result_all = calculate_reliability_metrics_all(merged_all, base_categories, suffixes_all)
    # Calculate reliability metrics for the subset of raters
    result_subset = calculate_reliability_metrics_all(merged_subset, base_categories, suffixes_subset)
    return result_all, result_subset, merged_all, merged_subset
...
#rating_columns = ['categories_selected', 'categories_selected_x', 'categories_selected_y', 'categories_selected_z']  # Adjust based on actual column names


# Define rating columns for all raters and subsets
#rating_columns_all = ['category_utilitarian', 'category_discretionary', 'category_landmark','category_evaluation']
rating_columns_all = ['category_utilitarian', 'category_utilitarian_x', 'category_utilitarian_y','category_utilitarian_z']
#rating_columns_subset = ['category_utilitarian_1', 'category_utilitarian_3', 'category_utilitarian']

# Generate report #1
rating_columns_subset = ['category_utilitarian', 'category_utilitarian_0', 'category_utilitarian_1']
report_all, report_subset1, merged_all, merged_subset1 = generate_reliability_report(file_paths, columns_of_interest, merge_keys, rating_columns_all, rating_columns_subset, subset_indices=[0,2,3]) # Humans only

# Generate report #2
rating_columns_subset = ['category_utilitarian', 'category_utilitarian_0']
report_all, report_subset2, merged_all, merged_subset2 = generate_reliability_report(file_paths, columns_of_interest, merge_keys, rating_columns_all, rating_columns_subset, subset_indices=[0,1]) # Lucas and AI

# Generate report #3
rating_columns_subset = ['category_utilitarian', 'category_utilitarian_0', 'category_utilitarian_1']
report_all, report_subset3, merged_all, merged_subset3 = generate_reliability_report(file_paths, columns_of_interest, merge_keys, rating_columns_all, rating_columns_subset, subset_indices=[1,2,3]) # Sarah and Sean and AI
...
merged_data.to_excel("merged_coding_data.xlsx", index=False)
...
rating_columns = ['categories_selected', 'categories_selected_y', 'categories_selected_z', 'categories_selected_{']  # Adjust based on suffixes
