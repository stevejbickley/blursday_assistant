...
def calculate_reliability_metrics(data, rating_columns):
    ...
    """
    # Prepare ratings matrix for Fleiss' Kappa
    ratings = data[rating_columns].apply(lambda row: row.value_counts(), axis=1).fillna(0)
    # Calculate Fleiss' Kappa
    fleiss_kappa_result = fleiss_kappa(ratings)
    # Calculate Cohen's Kappa for each pair of raters
    cohen_kappas = {}
    for i, col1 in enumerate(rating_columns):
        for j, col2 in enumerate(rating_columns[i + 1:], start=i + 1):
            pair_key = f"{col1} vs {col2}"
            cohen_kappas[pair_key] = cohen_kappa_score(data[col1].fillna(""), data[col2].fillna(""))
    # Percentage agreement (row-wise)
    percentage_agreement = data[rating_columns].apply(
        lambda row: row.nunique() == 1, axis=1
    ).mean() * 100
    return fleiss_kappa_result, cohen_kappas, percentage_agreement
