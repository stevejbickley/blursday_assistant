...
task_data_combined = grouped_df.pivot_table(
    ...
    values=["category_utilitarian_count", "category_discretionary_count", "category_landmark_count",
            "category_evaluation_count", "category_other_count"],
    aggfunc='sum'
)
...
#bar_width = 0.8 / len(color_map)  # Spread each category's bars within a total width of 0.8 per x-tick
...
for i, (category, color) in enumerate(color_map.items()):
    ...
    if future_fluency_col in task_data_combined.columns and past_fluency_col in task_data_combined.columns:
        ...
        #position = x + (i - len(color_map) / 2) * bar_width
        ...
        #future_bars = ax.bar(position, task_data_combined[future_fluency_col], bar_width, label=category_name, color=color)
        #past_bars = ax.bar(position, task_data_combined[past_fluency_col], bar_width, color=color)
...
# Plot lines connecting the tops of both Future and Past bars for each color group
for i, (category, color) in enumerate(color_map.items()):
    # Connect future bars
    ax.plot(x, bar_tops_future[category], color=color, linewidth=1, linestyle='--', alpha=0.7, label="")
    # Connect past bars
    ax.plot(x, bar_tops_past[category], color=color, linewidth=1, linestyle='--', alpha=0.7, label="")
...
ax.set_ylabel("Count")
ax.set_xticks(x)
#ax.set_xticklabels(task_data_combined.index)
