...
X = subset[["Sex", "Age", "Country", "Screen_Number", "Stringency_Index",
...
# Select the columns to convert to numeric and
...
#for dependent_var in ["category_utilitarian", "category_discretionary", "category_landmark"]: # "category_evaluation", "category_other", "total_category"
    for condition in ["Past Fluency", "Future Fluency"]:
        #for screen in [1, 2, 3]:
            ...
            #model = sm.NegativeBinomial.from_formula(f"{dependent_var} ~ {independent_vars}", data=analysis_data).fit(maxiter=100, disp=True)#.fit()
            #model = sm.NegativeBinomial.from_formula(f"{dependent_var} ~ {independent_vars}", data=subset).fit(maxiter=100, disp=True)#.fit()
            model = sm.NegativeBinomial.from_formula(f"{dependent_var} ~ {independent_vars}", data=subset).fit()
