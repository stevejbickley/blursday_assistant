...
def parse_dataframe(df):
    ...
    parsed_rows = []
    for index, row in df.iterrows():
        parsed_response = clean_up_agent_dump(response_dump=row['dump'], thread_id=row['thread_id'], run_id=row['run_id'], chunked_messages=row['chunked_messages'])
        parsed_rows.append(parsed_response)
    # Create a DataFrame from parsed rows
    parsed_df = pd.DataFrame(parsed_rows)
    # Expand lists in the DataFrame
...
def expand_lists_in_df(df, cols_to_expand):
    ...
    return df.explode(cols_to_expand)
...
# Summary statistics for the specified columns
summary_stats = clean_df[['categories', 'ranking', 'primary_codes', 'secondary_codes']].describe(include='all')
print(summary_stats)
