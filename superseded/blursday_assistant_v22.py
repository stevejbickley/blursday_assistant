    ...
    def get_ranking_other(row):
        ...
        for rank in rankings:
            ...
            if not any(re.search(pattern, rank) for pattern in ranking_patterns.values()):
                ...
                # NOTE: need to add another try/except statement here... try first to retrieve the number but if no number then return list/index number from .split(',') operation
                except ValueError:
                    continue
...
clean_df2 = add_category_and_ranking_columns(clean_df)

# Now write the clean data out to xlsx file
with pd.ExcelWriter(file_path[:-5]+"_clean_wide.xlsx", engine='openpyxl') as writer:
    clean_df2.to_excel(writer, index=False)

# Apply the function to add new columns to clean_df
clean_df3 = add_primary_code_columns(clean_df2,'primary')
clean_df3 = add_primary_code_columns(clean_df3,'secondary')

# Now write the clean data out to xlsx file
with pd.ExcelWriter(file_path[:-5]+"_clean_wide_final.xlsx", engine='openpyxl') as writer:
    clean_df3.to_excel(writer, index=False)

######### ------ START OF OLD CLEAN-UP CODE (Assistants API with hard coded regex) ------ #########
...
def add_category_and_ranking_columns(df):
    ...
    for col, pattern in category_patterns.items():
        df[col] = df['categories_selected'].apply(lambda x: 1 if any(re.search(pattern, str(item)) for item in (x if isinstance(x, list) else [x])) else 0)
    # Define patterns for ranking - NOTE: this needs work..
    ranking_patterns = {
    ...
    # Add missing columns for categories and rankings
    df['category_missing'] = df['categories_selected'].apply(lambda x: 1 if not x or (isinstance(x, str) and x.strip() == '') or (isinstance(x, list) and len(x) == 0) else 0)
...
clean_df2[['categories_selected','ranking','category_utilitarian','category_utilitarian','category_discretionary','category_landmark',
...
# Read in the "raw" coded data
clean_df = pd.read_excel(file_path[:-5]+"_clean_wide.xlsx") # just read the full dataset
