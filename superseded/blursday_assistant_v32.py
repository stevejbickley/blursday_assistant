...
analysis_df = analysis_df.dropna(subset=["Screen_Number", "original_response_lower"]) # 69603 rows down to 69421 rows
analysis_df = analysis_df.drop_duplicates(subset=['Experiment_ID', 'PID', 'UTC_yyyymmdd', 'Response_translated_lower', 'Screen_Number']) # 52492 rows

# (Optional) Write the resulting dataframe out to csv
#analysis_df.to_csv("main_data_for_analysis.csv", index=False)

# Remove specific variables or DataFrames
del blursday, blursday_nomatch, file_path, parsed_responses, analysis_df2
gc.collect()
...
len(analysis_df) # 68799 rows (old), 70722 rows (new w/ analysis_df2), 52492 rows (new w/ dropna's added)

# Drop rows where either the 'Screen_Number' or 'Task_Name' columns are NaN
analysis_df = analysis_df.dropna(
    subset=["Screen_Number", "Task_Name"]) # 52492 rows (new w/ dropna's added), 66762 rows (old) after this line
...
len(analysis_data) # 52492 rows (new w/ dropna's added), 66762 rows (old) after this line
...
# Check length of analysis_data
len(analysis_data) # 49589 rows (new w/ dropna's added), 63750 rows (old) after this line
...
#for dependent_var in ["category_utilitarian", "category_discretionary", "category_landmark"]: # "category_evaluation", "category_other", "total_category"
    for condition in ["Past Fluency", "Future Fluency"]:
        ...
        if regression_type == "OLS":
            ...
            # Define the Negative Binomial model (i.e., without setting alpha)
...
#with pd.ExcelWriter("results_for_publication_negativeBinomial.xlsx") as writer:
with pd.ExcelWriter("results_for_publication_OLS.xlsx") as writer:
...
for _, entry in results_df.iterrows():  # Iterate over rows in results_df
    ...
    #r_squared = row["R-squared"] # OLS only
    pseudo_r_squared = entry["Pseudo R-squared"] # Negative Binomial only...
    nobs = entry["nobs"]
    for predictor, coef in entry["Coefficient"].items():
        ...
        rows.append({
            ...
            #"R-squared": f"{r_squared:.3f}", # OLS only
            "Pseudo R-squared": f"{pseudo_r_squared:.3f}", # Negative Binomial only...
...
for _, entry in results_df.iterrows():
    dependent_var = entry["Dependent Variable"]
    condition = entry["Condition"]
    pseudo_r_squared = f"{entry['Pseudo R-squared']:.3f}"
    nobs = int(entry["nobs"])
    metrics.append({
        "Metric": "Pseudo R-squared",
        "Dependent Variable": dependent_var,
        "Condition": condition,
        "Value": pseudo_r_squared
    })
    metrics.append({
...
print(combined_df)
#combined_df.to_excel("final_regression_results_negativeBinomial.xlsx", index=False, sheet_name="Regression Results")
combined_df.to_excel("final_regression_results_OLS.xlsx", index=False, sheet_name="Regression Results")
