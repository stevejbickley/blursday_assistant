...
for i in range(0,len(created_thread_ids)):
    ...
    filtered_responses = [response for response in parsed_responses if
                          not all(value == [] or value == "" for value in response.values())]
    # Define a standard set of keys/columns you expect
    expected_keys = {'original_data', 'categories_selected', 'ranking', 'chosen_labels'}
    # Ensure each dictionary has all the expected keys, insert None for missing keys
    standardized_responses = [{key: response.get(key, None) for key in expected_keys} for response in
                              filtered_responses]
    ...
    cols_to_expand = ['categories_selected', 'ranking', 'chosen_labels', 'original_data']
