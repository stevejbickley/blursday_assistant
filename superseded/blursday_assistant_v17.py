...
def clean_up_agent_dump(response_dump, thread_id, run_id, chunked_messages):
    ...
    # Predefined activities to search for in categories
    category_activities = ["Discretionary Activities", "Temporal Landmark", "Utilitarian Activities", "Evaluations"]
    # Attempt to match patterns for each expected tag
    tags_found = {}
    for tag in ['raw_data', 'categories', 'ranking', 'primary_codes', 'secondary_codes', 'flagged',
                'justification_comments']:
        ...
        if matches:
            ...
            if regex_matches:
                regex_clean = [reg for reg in regex_matches]
    ...
    # Need to then e.g. str.replace('"','') for all matches in each of the above lists - OPTIONAL
    # Check if any of the tags were not found and print if so
    #if not all(tags_found.values()):
    #    print('\n\nNot all tags were found:\n\n')
    #    print(tags_found)
    #    print('\n\nIn the following raw data:\n' + response_dump + '\n\nFor thread ID: ' + thread_id + ' and run ID: ' + run_id + '\n')
    # Add the thread and run IDs back into the parsed_data dictionary
...
with pd.ExcelWriter(file_path[:-5]+"_clean.xlsx", engine='openpyxl') as writer:
    parsed_df.to_excel(writer, index=False)
