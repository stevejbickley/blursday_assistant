...
def load_and_merge_datasets(file_paths, columns_of_interest, merge_keys):
    """
    ...
    for i, dataset in enumerate(datasets[1:], start=1):
        ...
        merged_data = pd.merge(
            merged_data, dataset, on=merge_keys, suffixes=('', suffix), how='outer'
        )
    # Subset for specific file paths (1st, 3rd, and 4th files)
    merged_subset = pd.merge(
        datasets[0],  # 1st file
        datasets[2],  # 3rd file
        on=["Experiment_ID", "PID", "Response_translated"],
        suffixes=('_1', '_3')
    )
    merged_subset = pd.merge(
        merged_subset,
    ...
    return merged_data, merged_subset
...
def generate_reliability_report(file_paths, columns_of_interest, merge_keys, rating_columns_all, rating_columns_subset):
    """
    Generate a detailed reliability report.
    """
    # Load and merge datasets
    merged_all, merged_subset = load_and_merge_datasets(file_paths, columns_of_interest, merge_keys)
    # Drop duplicates
...
rating_columns_all = [
    'category_utilitarian', 'category_discretionary', 'category_landmark',
    'category_evaluation'
] # rating_columns_all = ['category_utilitarian', 'category_utilitarian_x', 'category_utilitarian_y','category_utilitarian_z']
rating_columns_subset = [
    'category_utilitarian_1', 'category_utilitarian_3', 'category_utilitarian'
]

# Generate report
report_all, report_subset, merged_all, merged_subset = generate_reliability_report(file_paths, columns_of_interest, merge_keys, rating_columns_all, rating_columns_subset) #
