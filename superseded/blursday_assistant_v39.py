...
del primary_columns, secondary_columns, categories, columns_to_sum, top_level_category, activity_list, count_activities, primary_columns_for_category, secondary_columns_for_category, primary_count, secondary_count, index, row, max_columns
gc.collect()
dir() # view all defined variables
...
grouped_df = analysis_data.copy().groupby(["Screen_Number", "Task_Name_clean"])[columns_to_sum].sum().reset_index()
...
# Function to calculate mean differences of categories at different group-levels
def calculate_mean_differences(df, group_by_columns, categories, column_to_compare="Task_Name_clean", values_to_compare=["Past Fluency","Future Fluency"]):
    results = []
    grouped_data = df.copy().groupby(group_by_columns)
    for group, group_df in grouped_data:
        # Split data into Past Fluency and Future Fluency for comparison
        past_fluency = group_df[group_df[column_to_compare] == values_to_compare[0]]
        future_fluency = group_df[group_df[column_to_compare] == values_to_compare[1]]
        for category in categories:
            if len(past_fluency) > 0 and len(future_fluency) > 0:
                # Calculate means
                ...
                significance = "***" if t_p_value < 0.01 else "**" if t_p_value < 0.05 else "*" if t_p_value < 0.1 else ""
                # Add results to list
                results.append({
                    "Group": group,
                    "Category": category,
...
categories = ["category_utilitarian_count", "category_discretionary_count",
              "category_landmark_count", "category_evaluation_count", "category_other_count",
              "category_utilitarian_mean", "category_discretionary_mean",
              "category_landmark_mean", "category_evaluation_mean", "category_other_mean"]

# Analysis at different grouping levels
grouping_levels = [
    ["Screen_Number", "Task_Name_clean"],
    ["Screen_Number", "Task_Name_clean", "Sex"],
    ["Screen_Number", "Task_Name_clean", "PID"]
]

# Calculate results for each grouping level and save to Excel
with pd.ExcelWriter("mean_differences_by_group.xlsx") as writer:
    for level in grouping_levels:
        result_df = calculate_mean_differences(analysis_data, level, categories)
        sheet_name = "-".join(level)[:30]  # Excel sheet name limit is 31 characters
        result_df.to_excel(writer, sheet_name=sheet_name, index=False)

print("Mean differences at all levels have been calculated and saved to 'mean_differences_by_group.xlsx'.")
...
categories = ["category_utilitarian_count", "category_discretionary_count", "category_landmark_count", "category_evaluation_count", "category_other_count", "category_utilitarian_mean", "category_discretionary_mean", "category_landmark_mean", "category_evaluation_mean", "category_other_mean"]
...
for main_category, subcategories in categories.items():
    ...
    for i, subcategory in enumerate(subcategories):
        ...
        #future_fluency_col = f"primary_{subcategory.lower().replace('-', '_').replace('/', '_').replace(' and ', '_').replace(' ', '_').replace('(', '').replace(')', '')}_count_Future Fluency"
        future_fluency_col = f"primary_{subcategory.lower().replace('-', '_').replace('/', '_').replace(' and ', '_').replace(' ', '_').replace('(', '').replace(')', '')}_mean_Future Fluency"
        #past_fluency_col = f"primary_{subcategory.lower().replace('-', '_').replace('/', '_').replace(' and ', '_').replace(' ', '_').replace('(', '').replace(')', '')}_count_Past Fluency"
        past_fluency_col = f"primary_{subcategory.lower().replace('-', '_').replace('/', '_').replace(' and ', '_').replace(' ', '_').replace('(', '').replace(')', '')}_mean_Past Fluency"
        if future_fluency_col in task_data_combined.columns and past_fluency_col in task_data_combined.columns:
    ...
    #ax.set_title(f"{main_category}: Total Counts by Sub-Category for Future Fluency and Past Fluency Tasks")
    ax.set_title(f"{main_category}: Mean Per Person by Sub-Category for Future Fluency and Past Fluency Tasks",fontsize=18)
    ax.set_ylabel("Count", fontsize=16)
    ax.set_xticks(x)
    ax.set_xticklabels(["1 year ago", "1 month ago", "1 week ago", "1 week ahead", "1 month ahead", "1 year ahead"], fontsize=14)
    # ax.legend(title="Category", loc="upper left", bbox_to_anchor=(0, 1))  # Move legend to top left
    ax.legend(title="Category", loc="upper right", bbox_to_anchor=(1, 1),fontsize=14)  # Move legend to top right
    plt.tight_layout()
...
#independent_vars = "Sex + Age + Country + Screen_Number + Stringency_Index + Reported_Loneliness + Felt_Loneliness + Subjective_Confinement + ConfDuration" #+ ConfDuration

# Sex and Sex/Loneliness/Confinement/etc Interactions
independent_vars1 = "Sex + Age + Screen_Number + Stringency_Index + Reported_Loneliness + Felt_Loneliness + Subjective_Confinement + ConfDuration" #+ ConfDuration
independent_vars2 = "Sex + Age + Screen_Number + Stringency_Index + Reported_Loneliness + Felt_Loneliness + Subjective_Confinement + ConfDuration + Sex:Stringency_Index + Sex:Reported_Loneliness + Sex:Felt_Loneliness + Sex:Subjective_Confinement + Sex:ConfDuration"
...
independent_vars2 = "Sex + Age + Screen_Number + Stringency_Index + Reported_Loneliness + Felt_Loneliness + Subjective_Confinement + ConfDuration + Extraversion + Agreeableness + Conscientiousness + Neuroticism + Openness + Sex:Extraversion + Sex:Agreeableness + Sex:Conscientiousness + Sex:Neuroticism + Sex:Openness"
...
independent_vars2 = "Sex + Age + Screen_Number + Stringency_Index + Reported_Loneliness + Felt_Loneliness + Subjective_Confinement + ConfDuration + PastNegative + PastPositive + PresentHedonistic + PresentFatalistic + Future+ Sex:PastNegative + Sex:PastPositive + Sex:PresentHedonistic + Sex:PresentFatalistic + Sex:Future"
independent_vars1 += f" + {session_fixed_effects}"
independent_vars2 += f" + {session_fixed_effects}"
...
independent_vars2 = "Sex + Age + Screen_Number + Stringency_Index + Reported_Loneliness + Felt_Loneliness + Subjective_Confinement + ConfDuration + Mindfulness_Score + Sex:Mindfulness_Score"
independent_vars1 += f" + {session_fixed_effects}"
independent_vars2 += f" + {session_fixed_effects}"
...
for dependent_var in ["utilitarian_activities_count_primary", "discretionary_activities_count_primary", "temporal_landmarks_count_primary"]: # "evaluations_count_primary", "total_count_primary", "total_count"
#for dependent_var in ["utilitarian_activities_count_secondary", "discretionary_activities_count_secondary", "temporal_landmarks_count_secondary"]: # "evaluations_count_secondary", "total_count_secondary", "total_count"
#for dependent_var in ["category_utilitarian", "category_discretionary", "category_landmark"]: # "category_evaluation", "category_other", "total_category"
    for condition in ["Past Fluency", "Future Fluency"]:
        ...
        if regression_type == "negative_binomial":
            ...
            poisson_model1 = smf.glm(f"{dependent_var} ~ {independent_vars1}", data=subset, family=sm.families.Poisson()).fit()
            # Step 2: Use the Poisson model's deviance to estimate alpha
            alpha_est1 = poisson_model1.deviance / poisson_model1.df_resid
            # Step 3: Re-fit with the Negative Binomial model using the estimated alpha
            model1 = smf.glm(f"{dependent_var} ~ {independent_vars1}", data=subset,family=sm.families.NegativeBinomial(alpha=alpha_est1)).fit()
            # Step 1: Fit a preliminary Poisson model
            poisson_model2 = smf.glm(f"{dependent_var} ~ {independent_vars2}", data=subset,family=sm.families.Poisson()).fit()
            # Step 2: Use the Poisson model's deviance to estimate alpha
            alpha_est2 = poisson_model2.deviance / poisson_model2.df_resid
            # Step 3: Re-fit with the Negative Binomial model using the estimated alpha
            model2 = smf.glm(f"{dependent_var} ~ {independent_vars2}", data=subset,family=sm.families.NegativeBinomial(alpha=alpha_est2)).fit()
            # Step 1: Fit a preliminary Poisson model
            ...
            mcfadden_r_squared1 = 1 - (model1.llf / null_model.llf) # maybe replace the cox-snell with alternative pseudo r-squared value (McFadden's)
            mcfadden_r_squared2 = 1 - (model2.llf / null_model.llf)  # maybe replace the cox-snell with alternative pseudo r-squared value (McFadden's)
            # Display model summary
            ...
            results.append({
                ...
                "Pseudo R-squared": mcfadden_r_squared1,  # Negative Binomial only...
                "nobs": model1.nobs  # Add number of observations
            })
            results.append({
                "Dependent Variable": dependent_var,
                "Condition": condition,
                ...
                # "Pseudo R-squared": cox_snell_r_squared, # Negative Binomial only...
                "Pseudo R-squared": mcfadden_r_squared2,  # Negative Binomial only...
                "nobs": model2.nobs  # Add number of observations
            })
            # Inspect the Log-Likelihood and Est. Alpha Values
            print(f"Model 1 Log-Likelihood: {model1.llf}")
            print(f"Null Model Log-Likelihood: {null_model.llf}")
            print(f"Estimated Alpha (Model 1): {alpha_est1}")
            # Inspect the Log-Likelihood and Est. Alpha Values
            print(f"Model 2 Log-Likelihood: {model2.llf}")
            print(f"Null Model Log-Likelihood: {null_model.llf}")
            print(f"Estimated Alpha (Model 1): {alpha_est2}")
...
for _, entry in results_df.iterrows():  # Iterate over rows in results_df
    ...
    if regression_type == "negative_binomial":
        prsquared = entry["Pseudo R-squared"] # Negative Binomial only...
    nobs = entry["nobs"]
...
for _, entry in results_df.iterrows():
    ...
    if regression_type == "negative_binomial":
        prsquared = f"{entry['Pseudo R-squared']:.3f}" # Negative Binomial only...
    nobs = int(entry["nobs"])
    if regression_type == "OLS":
        metrics.append({
            "Metric": "R-squared",
...
def generate_descriptive_stats(analysis_data):
    ...
    # Sex distribution (categorical example)
    ...
    # Country distribution (categorical example)
    ...
    # Return both tables
    return content_analysis_stats, sex_stats, country_stats, participant_numeric_stats


# Generate the descriptive stats
content_analysis_stats, sex_stats, country_stats, participant_numeric_stats = generate_descriptive_stats(analysis_data)
