...
def clean_up_agent_dump(response_dump, thread_id, run_id, chunked_messages):
    ...
    # Store the entire response in 'dump'
    parsed_data['dump'].append(response_dump)
    ...
    raw_data_matches = list(filter(None, re.findall(raw_data_pattern, chunked_messages, re.DOTALL)))
    category_matches = list(filter(None, re.findall(category_pattern, response_dump, re.DOTALL)))
    ranking_matches = list(filter(None, re.findall(ranking_pattern, response_dump, re.DOTALL)))
    primary_codes_matches = list(filter(None, re.findall(primary_codes_pattern, response_dump, re.DOTALL)))
    secondary_codes_matches = list(filter(None, re.findall(secondary_codes_pattern, response_dump, re.DOTALL)))
    flagged_matches = list(filter(None, re.findall(flagged_pattern, response_dump, re.DOTALL)))
    justification_matches = list(filter(None, re.findall(justification_pattern, response_dump, re.DOTALL)))

    # Attempt to match patterns for each expected tag
    tags_found = {}
    for tag in ['raw_data', 'categories', 'ranking', 'primary_codes', 'secondary_codes', 'flagged',
                'justification_comments']:
        matches = re.findall(f"<{tag}>(.*?)</{tag}>", response_dump)
        if matches:
            # Flatten the list of matches into the parsed_data dictionary
            parsed_data[tag].extend(matches)
            tags_found[tag] = True
            ...
            if tag == 'raw_data':
                regex_pattern = raw_data_pattern
                regex_matches = list(filter(None, re.findall(regex_pattern, chunked_messages, re.DOTALL)))
            else:
                if tag == 'categories':
                    regex_pattern = category_pattern
                elif tag == 'ranking':
                    regex_pattern = ranking_pattern
                elif tag == 'primary_codes':
                    regex_pattern = primary_codes_pattern
                elif tag == 'secondary_codes':
                    regex_pattern = secondary_codes_pattern
                elif tag == 'flagged':
                    regex_pattern = flagged_pattern
                elif tag == 'justification_comments':
                    regex_pattern = justification_pattern
                regex_matches = list(filter(None, re.findall(regex_pattern, response_dump, re.DOTALL)))
            if regex_matches:
                parsed_data[tag].extend(regex_matches)
                tags_found[tag] = True
            else:
                tags_found[tag] = False


    # Need to then e.g. str.replace('*','') for all matches in each of the above lists - OPTIONAL
    # Need to then e.g. str.replace('"','') for all matches in each of the above lists - OPTIONAL
